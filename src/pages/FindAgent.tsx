...
const FindAgent = () => {
   ...
   const fetchAgents = async () => {
      setLoading(true);
      try {
         // 1. Fetch profiles where role is Broker, Agent, or Seller
         const { data, error } = await supabase
            .from('profiles')
            .select('*')
            .in('role', ['Broker', 'Agent', 'Seller']); // ✅ Now includes Sellers too!
         ...
         if (data) {
            ...
            const mappedAgents: Agent[] = data.map((u: any) => ({
               ...
               location: 'Noida', // Default for now
               topAreas: ['Sector 150', 'Sector 137', 'Greater Noida'], // Placeholder
               languages: ['English', 'Hindi'],
               verified: u.is_verified || true, // Defaulting to true for UI feel
               email: u.email,
               phone: u.phone || ''
            }));
                     ...
                     <div key={agent.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-shadow p-6 flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-shrink-0 flex flex-col items-center md:items-start md:w-48">
                           ...
                           <div className="text-center md:text-left">
                              <h3 className="font-bold text-lg text-gray-900 capitalize">{agent.name}</h3>
